...
#TODO:  vpc_zone_identifier = ["${coalescelist(var.cluster_subnet_ids, data.aws_subnet_ids.selected.ids)}"]
...
resource "aws_instance" "bootstrap_node" {
  ...
  key_name             = var.key_name
  subnet_id = element(coalesce(var.cluster_subnet_ids, data.aws_subnet_ids.selected.ids),0)
