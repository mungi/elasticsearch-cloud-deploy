...
resource "aws_autoscaling_group" "client_nodes" {
  ...
  default_cooldown     = 30
  force_delete         = true
  launch_configuration = aws_launch_configuration.client[0].name
  health_check_type    = var.health_check_type

  load_balancers = aws_elb.es_client_lb.*.name

#TODO: vpc_zone_identifier   = ["${coalescelist(var.clients_subnet_ids, list(data.aws_subnet_ids.selected.ids[0]))}"]
  vpc_zone_identifier = coalesce(data.aws_subnet_ids.selected.ids)
